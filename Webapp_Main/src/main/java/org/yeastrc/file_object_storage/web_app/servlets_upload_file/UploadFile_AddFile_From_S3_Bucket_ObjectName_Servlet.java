...
/**
 * Accept a AWS S3 Bucket name and Object Name
 * 
 *  This Servlet should be considered a webservice as it returns JSON or XML
 */
public class UploadFile_AddFile_From_S3_Bucket_ObjectName_Servlet extends HttpServlet {

	private static final Logger log = LoggerFactory.getLogger( UploadFile_AddFile_From_S3_Bucket_ObjectName_Servlet.class );
...
//	private static final String STANDARD_UPLOAD_FILE_CONTENTS_TEMP_FILENAME = "temp_uploaded_file_contents";
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		UploadFile_AddFileIn_S3_BucketName_ObjectName_Request uploadFile_AddFileIn_S3_BucketName_ObjectName_Request = null;
		...
		try {
			...
			try {
				uploadFile_AddFileIn_S3_BucketName_ObjectName_Request = (UploadFile_AddFileIn_S3_BucketName_ObjectName_Request) requestObj;
			} catch (Exception e) {
				String msg = "Failed to cast requestObj to UploadFile_AddFileIn_S3_BucketName_ObjectName_Request";
				log.error( msg, e );
				throw new FileObjectStorageBadRequestToServletException( e );
			}
...
//		log.warn("INFO:: uploadFile_AddFileIn_S3_BucketName_ObjectName_Request: " + uploadFile_AddFileIn_S3_BucketName_ObjectName_Request );
		
		try {
		
			UploadFile_AddFileIn_S3_BucketName_ObjectName_Response webserviceResponse_Object = 
					processRequest( uploadFile_AddFileIn_S3_BucketName_ObjectName_Request, request, response );
	...
	/**
	 * @param uploadFile_AddFileIn_S3_BucketName_ObjectName_Request
	 * @param request
	 * @param response
	 * @throws Exception 
	 */
	private UploadFile_AddFileIn_S3_BucketName_ObjectName_Response processRequest( 
			
			UploadFile_AddFileIn_S3_BucketName_ObjectName_Request uploadFile_AddFileIn_S3_BucketName_ObjectName_Request,
			HttpServletRequest request, 
			HttpServletResponse response) throws Exception {
		
		try {
			UploadFile_AddFileIn_S3_BucketName_ObjectName_Response webserviceResponse = new UploadFile_AddFileIn_S3_BucketName_ObjectName_Response();
...
//			}
			...
			boolean gzipCompressTheStoredFile = uploadFile_AddFileIn_S3_BucketName_ObjectName_Request.isGzipCompressTheStoredFile();
			
			long submitted_fileSize_AsLong = 0; 

			if ( StringUtils.isEmpty( uploadFile_AddFileIn_S3_BucketName_ObjectName_Request.getS3Bucket() ) ) {
				String msg = "request is missing s3Bucket ";
				log.warn( msg );
				throw new FileObjectStorageBadRequestToServletException( msg );
			}

			if ( StringUtils.isEmpty( uploadFile_AddFileIn_S3_BucketName_ObjectName_Request.getS3ObjectKey() ) ) {
				String msg = "request is missing s3ObjectKey ";
				log.warn( msg );
				throw new FileObjectStorageBadRequestToServletException( msg );
			}
			...
			
			{
				...
				S3Client amazonS3_Client_ForInput = null;

				{  // Use Region from Config, otherwise SDK use from Environment Variable

					if ( StringUtils.isNotEmpty( uploadFile_AddFileIn_S3_BucketName_ObjectName_Request.getS3Region() ) ) {

						Region aws_S3_Region = Region.of(uploadFile_AddFileIn_S3_BucketName_ObjectName_Request.getS3Region());

						amazonS3_Client_ForInput = 
								S3Client.builder()
				...
				//  Read the S3 Object to compute the hash
				...
				GetObjectRequest getObjectRequest = 
						GetObjectRequest
						.builder()
						.bucket(uploadFile_AddFileIn_S3_BucketName_ObjectName_Request.getS3Bucket())
						.key(uploadFile_AddFileIn_S3_BucketName_ObjectName_Request.getS3ObjectKey())
						.build();
				
				try ( 
						ResponseInputStream<GetObjectResponse> getObjectResponseMainObject_UsableAs_InputStream =
						amazonS3_Client_ForInput.getObject(getObjectRequest) ) {

					GetObjectResponse getObjectResponse = getObjectResponseMainObject_UsableAs_InputStream.response();

					submitted_fileSize_AsLong = getObjectResponse.contentLength();
								
				       // file upload size limit
					if ( submitted_fileSize_AsLong > FileUploadConstants.MAX_FILE_UPLOAD_SIZE ) {

						log.warn( " File Exceeds allowed size.  File size: " + submitted_fileSize_AsLong
								...
								+ ", submitted Bucket: " 
								+ uploadFile_AddFileIn_S3_BucketName_ObjectName_Request.getS3Bucket() 
								+ ", Object Key: " + uploadFile_AddFileIn_S3_BucketName_ObjectName_Request.getS3ObjectKey() );
						
						UploadFile_AddFileIn_S3_BucketName_ObjectName_Response uploadResponse = new UploadFile_AddFileIn_S3_BucketName_ObjectName_Response();
					...
					log.warn("Provided AWS S3 Bucket and Object Key does not exist: Bucket: " 
							+ uploadFile_AddFileIn_S3_BucketName_ObjectName_Request.getS3Bucket()
							+ ", Object Key: " + uploadFile_AddFileIn_S3_BucketName_ObjectName_Request.getS3ObjectKey() );
					...
					String msg = "Failed reading submitted AWS S3 Bucket and Object Key.  Bucket: " 
					+ uploadFile_AddFileIn_S3_BucketName_ObjectName_Request.getS3Bucket()
					+ ", Object Key: " + uploadFile_AddFileIn_S3_BucketName_ObjectName_Request.getS3ObjectKey();
					log.error(msg, e);
			...
			if ( StringUtils.isNotEmpty( ConfigData_Directories_ProcessUploadInfo_InWorkDirectory.getSingletonInstance().getS3Bucket_MainStorage() ) ) {
				...
				this._saveTo_AWS_S3(
						...
						uploadFile_AddFileIn_S3_BucketName_ObjectName_Request
						);;
				...
				throw new FileObjectStorageWebappInternalException( "NOT Supported for now 'Save to Local File System'");
...
//				HeadObjectResponse headObjectResponse = 
		...
		if ( ( ! gzipCompressTheStoredFile ) && submitted_fileSize_AsLong < YRC_FileObjectStorage__AWS_S3_Constants.AWS_S3_MAXIMUM_PUT_OBJECT_OR_UPLOAD_PART_SIZE ) {
			...
			try ( 
					ResponseInputStream<GetObjectResponse> getObjectResponseMainObject =
					amazonS3_Client_ForInput.getObject(getObjectRequest) ) {

//				GetObjectResponse getObjectResponse = getObjectResponseMainObject.response();

				InputStream inputStreamFromTempObject = getObjectResponseMainObject;

	            PutObjectRequest putObjectRequest = PutObjectRequest.builder()
	                .bucket(amazonS3_bucketName)
	                .key(s3_Object_Key)
	                .contentLength( submitted_fileSize_AsLong )
	                .build();
	            
	            RequestBody amazonS3_RequestBody = RequestBody.fromInputStream(inputStreamFromTempObject, submitted_fileSize_AsLong);
			...
			try {
				...
				{
					final long submittedFileWithPath_FileObject__Length = submitted_fileSize_AsLong;
